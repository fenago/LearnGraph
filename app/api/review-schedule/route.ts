...
export async function GET(request: NextRequest) {
  try {
    ...
    const validPerformance = ['failed', 'difficult', 'good', 'easy'] as const;
    const performanceValue = performance && validPerformance.includes(performance as typeof validPerformance[number])
      ? (performance as 'failed' | 'difficult' | 'good' | 'easy')
      : undefined;

    const schedule = db.scheduleNextReview(state, concept, performanceValue);
